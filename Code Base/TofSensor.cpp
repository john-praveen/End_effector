...
// You can adjust FoV if your sensor uses a different value
...
void read_tof()
{
    ...
    while (true)
    {
        ...
        {
            uart1.read(); // consume second header byte
            break;
        }
    }
    ...
    // Parse and print corner distances (indexes: 0, 7, 56, 63)
    int dataStart = 7; // Skip ID (1), timestamp (3), and some flags (3)
...
// Estimate the angle (in degrees) of the object relative to the center
// from 4 corner distances (in mm or cm — consistent units)
float calculateAngleFromCorners(float d0, float d7, float d56, float d63)
{
    ...
    // Optional: return horizontal only, or both as struct if needed
    return horizontalAngle; // or verticalAngle
}
